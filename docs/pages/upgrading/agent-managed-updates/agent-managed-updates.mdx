...
Once the `autoupdate_config` and `autoupdate_version` resources exist and the
update rollout has been created, installation artifacts generated by the
Teleport Proxy Service follow Managed Updates automatically: agent join
scripts generated by the Web UI during resource enrollment, the
`/scripts/install.sh` installation script, and the default installer script
used by server auto-discovery all install agents with the `teleport-update`
binary, enrolled in Managed Updates and following your cluster's target
version. This requires no extra flags or installer customization, on
self-hosted clusters as well as on cloud-hosted Teleport Enterprise.
