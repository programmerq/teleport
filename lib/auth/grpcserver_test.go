...
func TestIsMFARequired(t *testing.T) {
	...
	for _, requiredRole := range []bool{true, false} {
		for _, requiredCluster := range []bool{true, false} {
			t.Run(fmt.Sprintf("requiredRole=%v,requiredCluster=%v", requiredRole, requiredCluster), func(t *testing.T) {
				// Set require_role_mfa flag on the role.
				roleOpt := role.GetOptions()
				roleOpt.RequireSessionMFA = requiredRole
				role.SetOptions(roleOpt)
				err = srv.Auth().UpsertRole(ctx, role)
				require.NoError(t, err)

				// Set require_role_mfa flag on the cluster.
				authPref.(*services.AuthPreferenceV2).Spec.RequireSessionMFA = requiredCluster
				err = srv.Auth().SetAuthPreference(authPref)
				require.NoError(t, err)
				...
				require.NoError(t, err)
				require.Equal(t, resp.Required, requiredRole || requiredCluster)
			})
		}
	}
}
