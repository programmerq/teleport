...
// GetGCPAuthToken returns authorization token that will be used as a password
// when connecting to Cloud SQL databases.
...
func (a *Auth) GetTLSConfig(ctx context.Context, sessionCtx *Session) (*tls.Config, error) {
	...
	if sessionCtx.Server.IsGCP() {
		// Cloud SQL certs have "<project-id>:<instance-id>" string in CommonName.
	...
	if sessionCtx.Server.IsRDS() || sessionCtx.Server.IsGCP() {
